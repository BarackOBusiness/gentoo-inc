...
# wlroots[drm,gles2-renderer,libinput,?,X?]
# enable  with X and vice versa
WLROOTS_CDEPEND="
...
if [[ ${PV} == 9999 ]] ; then
	CDEPEND+="
		system-wlroots? ( gui-libs/wlroots:0/9999[drm(+),libinput(+)] )
	"
else
	CDEPEND+="
		system-wlroots? ( gui-libs/wlroots:0/16[drm(+),libinput(+)] )
	"
fi
